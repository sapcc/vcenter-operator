...
class VersionMismatchError(Exception):
    """Raised when a version mismatch is encountered."""
    ...
    def _get_latest_active_service_user_version(
        ...
        for version in reversed(service_user_versions):
            if version in vcenter_service_user_tracker[service_name][host]:
                return version
        raise VersionMismatchError(f"No vault version {service_user_versions} matches the current"
                                   f"pod version {vcenter_service_user_tracker[service_name][host].keys()}")
