...
def test_retrieve_mount_points(vault):
    mount_point_write_nsxt = "new_mount_point"
    vault.set_mount_point_write(mount_point_write_nsxt, service="test_service")
    ...
    assert DEFAULT_WRITE_MOUNT_POINT == vault.get_mountpoint(read=False, service_type="does-not-exist")
    assert mount_point_write_nsxt == vault.get_mountpoint(read=False,service_type="test_service")
