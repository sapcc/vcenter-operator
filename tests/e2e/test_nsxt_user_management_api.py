...
def test_check_nsxt_service_user(delete_user, user_api, configurator):
    user = os.getenv('NSXT_USER')
    password = os.getenv('NSXT_PW')
    bb = os.getenv('NSXT_BB')
    region = os.getenv('NSXT_REGION')
    ...
    configurator._check_service_user_nsxt(user_prefix, service, region, bb, path,
                                          latest_version, management_user, USERGROUP)


def test_dry_run(user_api):
    user_api.dry_run = True
    user_api.create_service_user(USERNAME, NEW_PASSWORD)
    users = user_api.list_users(prefix=USERNAME)
    assert 0 == len(users)
